...
+ (BOOL) libraryCreateDirectoryPath:(NSString *)strPath
{
    ...
    {
        //DEBUGINFO(@"1111111111111 %@", [array objectAtIndex:i]);
        if( i > 0)
        {
            ...
            {
                // DEBUGINFO(@"createfile failed at path %@", [[FileManagerController documentPath] stringByAppendingPathComponent:strSubPath]);
            }
        }
        else
        {
            ...
            {
                // DEBUGINFO(@"createfile failed at path %@", [[FileManagerController documentPath] stringByAppendingPathComponent:[array objectAtIndex:0]]);
            }
        }
...
+ (BOOL)copyItem:(NSString *)strFrom toItem:(NSString *)strTo
{
    ...
    if(error)
    {
        DEBUGINFO(@"copy %@ to %@ failed, error:%@", strFrom, strTo, error.description);
        return NO;
    }
    return YES;
}
...
+ (BOOL)removeFile:(NSString *)strFullpath
{
    ...
    if(error)
    {
        DEBUGINFO(@"remove file %@ failed ! error %@ ", strFullpath, error);
        return NO;
    }
    return YES;
}
...
+ (BOOL)moveFile:(NSString *)fromPath toPath:(NSString *)toPath
{
    ...
    if(error)
    {
        DEBUGINFO(@"copy %@ to %@ failed, error:%@", fromPath, toPath, error.description);
        return NO;
    }
    return YES;
}
