...
- (NSString *)appName {
//    return [NSString stringWithFormat:@"%@", @"Zine"];
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    //DEBUGINFO(@"%@", [self modelName]);
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:[self modelName] withExtension:@"momd"];
    //DEBUGINFO(@" %@", modelURL);
    //NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"M6s" withExtension:@"momd"];
...
-(void) setUpPersistentStoreCoordinator {
   ...
   //NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"M6s_development_5.sqlite"];
    NSError *error = nil;
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:options error:&error])
        DEBUGINFO(@"ERROR IN PERSISTENT STORE COORDINATOR! %@, %@", error, [error userInfo]);
}
...
- (BOOL)saveContext {
    ...
    if (![self.managedObjectContext save:&error]) {
        DEBUGINFO(@"Unresolved error in saving context! %@, %@", error, [error userInfo]);
        return NO;
    }
